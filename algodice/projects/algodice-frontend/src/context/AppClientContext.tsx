...
import { AlgodiceClient, AlgodiceFactory } from '../contracts/Algodice'
...
// TODO: Currently Hard coded for local testing.
const APP_ID = BigInt(1015)

type AppClientContextType = {
  appClient: AlgodiceClient | null
  error: Error | null
}
...
export const AppClientProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [appClient, setAppClient] = useState<AlgodiceClient | null>(null)
  ...
  useEffect(() => {
    ...
    if (!activeAddress || !stableSigner) {
      console.log("no active address or no stable signer")
      setAppClient(null)
      return
    }
    setError(null)
